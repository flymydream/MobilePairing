                      ...
                      delegate:(id<YBPopupMenuDelegate>)delegate
{
    if (self = [super init]) {
        ...
        kArrowHeight = 0;//10 如果需要箭头 则设置高度
        kArrowWidth = 0;//15 如果需要箭头 则设置宽度
        kButtonHeight = 44;
        kCornerRadius = 5.0;
        ...
        _contentView.bounces = titles.count > 2 ? YES : NO;
...
-(void)setIsCustomerKeyboard:(BOOL)isCustomerKeyboard {
    _isCustomerKeyboard = isCustomerKeyboard;
    if (_isCustomerKeyboard) {
        NSLog(@"菜单自定义键盘");
        _bgView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight*0.67);
    } else {
        NSLog(@"菜单不是自定义键盘");
...
-(void)setMenuWidth:(CGFloat)menuWidth {
    ...
    self.width = _menuWidth;
    _mainView.frame = self.bounds;
    _contentView.frame = _mainView.bounds;
    _contentView.height -= 2 * kArrowHeight;
    _contentView.centerY = _mainView.centerY;
...
-(void)setTitles:(NSArray *)titles {
    ...
    _contentColor = [UIColor whiteColor];
    _separatorColor = [UIColor lightGrayColor];
    self.width = self.menuWidth;
    self.height = (titles.count > 5 ? 5 * kButtonHeight : titles.count * kButtonHeight) + 2 * kArrowHeight;
    kArrowPosition = 0.5 * self.width - 0.5 * kArrowWidth;
    _titles = titles;
    if (self.isNeedHidden && self.isCustomerKeyboard) {
        self.alpha = 0;
        self.bgView.alpha = 0;
    } else {
        if ([titles count]) {
...
- (void)show {
    [kMainWindow removeFromSuperview];
