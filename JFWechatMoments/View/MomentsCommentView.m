...
- (void)setComments:(NSArray *)comments {
    ...
    if (_comments && _comments.count > 0) {
        ...
        for (int i = 0; i < _comments.count; i++) {
            Comment *commentObj = _comments[i];
            if (commentObj.attributedContent == nil) {
                commentObj.attributedContent = [self getAttributedContent:commentObj];
            }
            UILabel *label = _commentLabelArray[i];
            label.hidden = NO;
            label.attributedText = commentObj.attributedContent;
...
- (NSMutableAttributedString *)getAttributedContent:(Comment *)model {
    NSString *content = [NSString stringWithFormat:@"%@：%@", model.sender.nick, model.content];
    NSMutableAttributedString *attString = [[NSMutableAttributedString alloc] initWithString:content];
    [attString setAttributes:@{NSForegroundColorAttributeName: [UIColor colorWithRed:77 / 255.0 green:93 / 255.0 blue:136 / 255.0 alpha:1.0]} range:NSMakeRange(0, model.sender.nick.length)];
    return attString;
}
